...
class FFmpegMetadataPP(FFmpegPostProcessor):
    ...
    def run(self, info):
        ...
        idx = 2
        while os.path.isfile(part_filename := prepend_extension(filename, f'part{idx}')):
            temp_filename = prepend_extension(part_filename, 'temp')
            self.to_screen(f'Downscaling part{idx}')
            self.run_ffmpeg(part_filename, temp_filename,
                itertools.chain(self._options(info['ext']), *options))
